...
import time
from playsound import playsound  # 音声再生ライブラリをインポート
import pygame
import subprocess
...
def image_loading():
    ...
    start_image = cv2.imread('gamestart3.png')
    end_image = cv2.imread('gameover3.png')
    clear_image = cv2.imread('gameclear1.png')  # ゲームクリアの画像
...
def obstacle_loading():
    ...
    global obstacle_speed1, obstacle_radius1
    # 障害物の初期設定
    ...
    # 障害物の速度をそれぞれ設定する
    obstacle_speed1 = 35  # 障害物1の移動速度


    return
# 衝突回数とタイマーなどのカウンターを定義する関数
def game_count_setup():
    global collision_count, collision_limit, countdown_time, start_time, delay_start_time
    global delay_duration, show_end_image, show_clear_image, bar_reduce, hp_bar_count
    # 衝突回数をカウントする変数
    collision_count = 0
    collision_limit = 100
...
pygame.mixer.music.load('gamebgm1.mp3')
...
while True:
    ...
    if results:
        ...
        for box, cls in zip(boxes, classes):
            if is_collision(obstacle_center_x1, obstacle_center_y1, obstacle_radius1, box):
                collision_count += 1
                if bar_reduce == True:
                    hp_bar_count -= 1
                    bar_reduce = False
    ...
    cv2.putText(annotatedFrame, f"|" * (hp_bar_count), (70, frame_height - 20), cv2.FONT_HERSHEY_PLAIN, 2.0, (255, 255, 255), 2, cv2.LINE_AA)
    ...
    if show_clear_image:
        ...
        # ユーザーの入力を待つ
        while True:
            cv2.putText(clear_image, "Press 'Y' for Yes or 'N' for No", (50, 50), cv2.FONT_HERSHEY_PLAIN, 2.0, (255, 255, 255), 2, cv2.LINE_AA)
            cv2.imshow('Webcam', clear_image)
            key = cv2.waitKey(1) & 0xFF

            if key == ord('y') or key == ord('Y'):  # 'Y'キーが押された場合
                # 別ファイルを開く
                import kansei4
                subprocess.run(["python", "kansei4.py"])  # test1.pyを実行
                break
            elif key == ord('n') or key == ord('N'):  # 'N'キーが押された場合
                close_window()  # ウィンドウを閉じる
                exit()  # プログラムを終了
            if cv2.waitKey(1) == 27:  # ESCキーで強制終了
                close_window()
                exit()
