...
def image_loading():
    ...
    start_image = cv2.imread('extra start (2).png')
...
def obstacle_loading():
    ...
    # 障害物の速度をそれぞれ設定する
    obstacle_speed1 = random.randint(10, 20)  # 障害物1の移動速度
    obstacle_speed2 = random.randint(5, 15)   # 障害物2の移動速度

    return
# 衝突回数とタイマーなどのカウンターを定義する関数
def game_count_setup():
    global collision_count, collision_limit, countdown_time, start_time, delay_start_time, delay_duration, show_end_image, show_clear_image, bar_reduce, hp_bar_count
    # 衝突回数をカウントする変数
    collision_count = 0
    collision_limit = 100
